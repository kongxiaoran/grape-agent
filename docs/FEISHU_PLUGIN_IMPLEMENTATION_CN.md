# Mini-Agent 飞书插件技术实现文档

## 1. 目标与范围

本文档说明在 Mini-Agent 中如何实现飞书（Feishu/Lark）长连接插件，使 Agent 能与飞书机器人进行**双向通信**：
...
- CLI 启动入口 `mini-agent-feishu`（`pyproject.toml`）
- 依赖 `lark-oapi`
...
参考 openclaw 的飞书通道思路，在 Mini-Agent 中采用以下原则：
...
- 持久化去重缓存（TTL + 最大条目）
- 文件：`~/.mini-agent/feishu/dedup.json`
...
- 根目录：`~/.mini-agent/feishu_workspaces`
- 子目录：`~/.mini-agent/feishu_workspaces/<chat_id>`
