...
编辑 `grape_agent/config/config.yaml`，至少填：
...
核心循环在 [agent.py](/Users/kxr/learning/Grape-Agent/grape_agent/agent.py)。
...
- [cli.py](/Users/kxr/learning/Grape-Agent/grape_agent/cli.py): 主入口，组装 runtime、启动 channel/gateway/cron
- [runtime_factory.py](/Users/kxr/learning/Grape-Agent/grape_agent/runtime_factory.py): 构建 LLM、工具、system prompt
- [config.py](/Users/kxr/learning/Grape-Agent/grape_agent/config.py): 配置模型定义、加载、校验、默认路径策略

### 3.2 Agent 核心

- [agent.py](/Users/kxr/learning/Grape-Agent/grape_agent/agent.py): 多步循环、工具执行、取消机制、消息压缩
- [schema.py](/Users/kxr/learning/Grape-Agent/grape_agent/schema/schema.py): Message、ToolCall、LLMResponse 等协议模型

### 3.3 会话与多代理

- [session_store.py](/Users/kxr/learning/Grape-Agent/grape_agent/session_store.py): 会话仓库 + 每会话互斥锁
- [orchestrator.py](/Users/kxr/learning/Grape-Agent/grape_agent/agents/orchestrator.py): 子代理 spawn/send/history/run
- [registry.py](/Users/kxr/learning/Grape-Agent/grape_agent/agents/registry.py): 多 agent profile 解析
- [resolver.py](/Users/kxr/learning/Grape-Agent/grape_agent/routing/resolver.py): 路由规则命中

### 3.4 通道与网关

- [runtime.py](/Users/kxr/learning/Grape-Agent/grape_agent/channels/runtime.py): 通道插件生命周期
- [plugin.py](/Users/kxr/learning/Grape-Agent/grape_agent/channels/plugins/feishu/plugin.py): Feishu 通道插件
- [bridge.py](/Users/kxr/learning/Grape-Agent/grape_agent/feishu/bridge.py): Feishu 消息 -> Agent 会话桥接
- [server.py](/Users/kxr/learning/Grape-Agent/grape_agent/gateway/server.py): Gateway TCP Server
- [handlers](/Users/kxr/learning/Grape-Agent/grape_agent/gateway/handlers): `health/status/sessions/channels/cron` 方法

### 3.5 Webterm 桥接

- [server.py](/Users/kxr/learning/Grape-Agent/grape_agent/webterm_bridge/server.py): HTTP API
- [session_manager.py](/Users/kxr/learning/Grape-Agent/grape_agent/webterm_bridge/session_manager.py): bridge session 管理与建议命令生成
- [gateway_client.py](/Users/kxr/learning/Grape-Agent/grape_agent/webterm_bridge/gateway_client.py): 桥接对 Gateway 的 TCP 客户端
...
核心逻辑在 [cli.py](/Users/kxr/learning/Grape-Agent/grape_agent/cli.py)。

---

## 5. Agent 主循环详解（最关键）

Agent 主循环在 [agent.py](/Users/kxr/learning/Grape-Agent/grape_agent/agent.py) 的 `run()`：
...
工具统一协议在 [base.py](/Users/kxr/learning/Grape-Agent/grape_agent/tools/base.py)：
...
- 文件工具：`read_file/write_file/edit_file` 见 [file_tools.py](/Users/kxr/learning/Grape-Agent/grape_agent/tools/file_tools.py)
- Shell 工具：`bash/bash_output/bash_kill` 见 [bash_tool.py](/Users/kxr/learning/Grape-Agent/grape_agent/tools/bash_tool.py)
...
装配代码在 [runtime_factory.py](/Users/kxr/learning/Grape-Agent/grape_agent/runtime_factory.py)。

---

## 7. 会话模型与并发安全（为什么不会串台）

会话键格式：`agent:{agent_id}:{channel}:{session_id}` 见 [session_key.py](/Users/kxr/learning/Grape-Agent/grape_agent/routing/session_key.py)。

每个会话 `AgentSession` 都带一把 `asyncio.Lock`，保证同会话串行执行。  
即使多个请求同时命中同一 session，也会顺序执行，避免消息历史错乱。

见 [session_store.py](/Users/kxr/learning/Grape-Agent/grape_agent/session_store.py)。
...
并通过 `max_depth` 和 leaf 工具禁用策略防止无限递归。见 [orchestrator.py](/Users/kxr/learning/Grape-Agent/grape_agent/agents/orchestrator.py)。

---

## 8. 通道插件架构（以 Feishu 为例）

通道接口协议在 [types.py](/Users/kxr/learning/Grape-Agent/grape_agent/channels/types.py)：
...
`ChannelRuntime` 负责按配置启动已启用插件。见 [runtime.py](/Users/kxr/learning/Grape-Agent/grape_agent/channels/runtime.py)。
...
核心实现见 [bridge.py](/Users/kxr/learning/Grape-Agent/grape_agent/feishu/bridge.py) 与 [plugin.py](/Users/kxr/learning/Grape-Agent/grape_agent/channels/plugins/feishu/plugin.py)。
...
监听在 `gateway.host:gateway.port`，默认 `127.0.0.1:8765`。实现见 [server.py](/Users/kxr/learning/Grape-Agent/grape_agent/gateway/server.py)。
...
注册入口见 [handlers/__init__.py](/Users/kxr/learning/Grape-Agent/grape_agent/gateway/handlers/__init__.py)。
...
- [webterm_bridge/server.py](/Users/kxr/learning/Grape-Agent/grape_agent/webterm_bridge/server.py)
- [webterm_bridge/session_manager.py](/Users/kxr/learning/Grape-Agent/grape_agent/webterm_bridge/session_manager.py)
...
- [models.py](/Users/kxr/learning/Grape-Agent/grape_agent/cron/models.py)
- [store.py](/Users/kxr/learning/Grape-Agent/grape_agent/cron/store.py)
- [scheduler.py](/Users/kxr/learning/Grape-Agent/grape_agent/cron/scheduler.py)
- [executor.py](/Users/kxr/learning/Grape-Agent/grape_agent/cron/executor.py)
- [delivery.py](/Users/kxr/learning/Grape-Agent/grape_agent/cron/delivery.py)

---

## 12. 配置系统（入门最容易踩坑的地方）

配置主文件一般是 `grape_agent/config/config.yaml`。  
查找优先级（高 -> 低）：

1. `./grape_agent/config/config.yaml`
2. `~/.grape-agent/config/config.yaml`
3. 包内默认配置

逻辑在 [config.py](/Users/kxr/learning/Grape-Agent/grape_agent/config.py)。
...
在 `grape_agent/tools/echo_upper_tool.py`：
...
在 [runtime_factory.py](/Users/kxr/learning/Grape-Agent/grape_agent/runtime_factory.py) 的工具初始化位置把它 append 进去。
...
1. 在 `grape_agent/gateway/handlers/` 新建 `ping.py`
2. 定义 `handle_ping(params, ctx, conn) -> dict`
3. 在 [handlers/__init__.py](/Users/kxr/learning/Grape-Agent/grape_agent/gateway/handlers/__init__.py) 注册：
...
1. 新建 `grape_agent/channels/plugins/<your_channel>/plugin.py`
2. 实现类并管理连接生命周期
3. 在 [runtime.py](/Users/kxr/learning/Grape-Agent/grape_agent/channels/runtime.py) 的 `build_default_registry()` 注册
4. 在 [config.py](/Users/kxr/learning/Grape-Agent/grape_agent/config.py) 增加对应配置模型和加载逻辑
